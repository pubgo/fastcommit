	...
	return &redant.Command{
		...
		Handler: func(ctx context.Context, i *redant.Invocation) error {
			...
			if pathutil.IsExist(".version") {
				assert.Must(os.Rename(".version", ".release"))
			}

			var ver *semver.Version
			if pathutil.IsExist(".release") {
				vv := strings.TrimPrefix(string(lo.Must1(os.ReadFile(".release"))), "v")
				...
				if len(tags) == 0 {
					ver = lo.Must1(semver.NewSemver(fmt.Sprintf("%s-%s.1", lo.Must1(os.ReadFile(".release")), selected)))
				} else {
			...
			if selected == envRelease {
				vv := string(lo.Must1(os.ReadFile(".release")))
